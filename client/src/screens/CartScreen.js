...
const CartScreen = ({ match, location, history }) => {
  //! react router variable ':id'
  const productId = match.params.id

  //! for '?qty=1' whatever is after the '?'
  ...
  const checkoutHandler = () => {
    // if not logged in, push to login page otherwise push to shipping page
              ...
              {cartItems.map(item => (
                <ListGroup.Item key={item.productId}>
                  <Row>
                    ...
                    <Col md={4}><Link to={`/product/${item.productId}`}>{item.name}</Link></Col>
